...
from keras.layers import Dense, Dropout, Embedding, LSTM, Bidirectional, Conv1D, GlobalAveragePooling1D, GRU, MaxPooling1D, AveragePooling1D, merge, GlobalMaxPooling1D
...
#word_vec_file='data/word_vectors/corpus_word2vec'
#word_vec_file='/data/dchaudhu/rel_ent_pred/lexvec.commoncrawl.300d.W.pos.vectors'
word_vec_file='data/word_vectors/word2vec_amazon'
...
lda_model = 'data/lda_models/amazon_lda'
saved_dict = 'data/lda_models/amazon_dict'
...
def get_domain_revs(domain, test_t):
    ...
    positive_reviews = pd.read_table(pos_revw_file, nrows=50000)
    negative_reviews = pd.read_table(neg_revw_file, nrows=50000)
...
def cnn_model(embedding_weights, cv_dat, max_len, idx2word):
    #CNN model from Wei et al.
    ...
    #print (train_x.shape)
    #print train_y.shape
    train_x, val_x, train_y, val_y = train_test_split(train_x, train_y, test_size=0.12, random_state=666)
    ...
    #conv1 = Conv1D(filters=128, kernel_size=3, padding='same', activation=relu, weights=layer_dict['conv1d_1'].get_weights())
    #conv2 = Conv1D(filters=128, kernel_size=5, padding='same', activation=relu, weights=layer_dict['conv1d_2'].get_weights())
    #conv3 = Conv1D(filters=128, kernel_size=7, padding='same', activation=relu, weights=layer_dict['conv1d_3'].get_weights())
  
    conv1 = Conv1D(filters=250, kernel_size=5, padding='same', activation='relu')
    conv2 = Conv1D(filters=250, kernel_size=5, padding='same', activation='relu')
    #conv3 = Conv1D(filters=100, kernel_size=7, padding='same', activation='relu')
 

    conv1a = conv1(embedded_layer_body)
    #glob1a = GlobalAveragePooling1D()(conv1a)
    max1 = MaxPooling1D()(conv1a)
    flat_m1 = Flatten()(max1)
    conv2a = conv2(max1)
    #glob2a = GlobalAveragePooling1D()(conv2a)
    max2 = MaxPooling1D()(conv2a) 
    #conv3a = conv3(embedded_layer_body)
    #glob3a = GlobalAveragePooling1D()(conv3a)
    #max3 = MaxPooling1D()(conv3a)
    flat_m2 = Flatten()(max2)
    #merge_pooling = concatenate([glob1a, glob2a, glob3a])
    #merge_pooling = merge([max1, max2, max3], mode='concat', concat_axis=1)
    #flatten = Flatten()(merge_pooling)
    #print flatten
    hidden_layer = Dense(250, activation='relu')(flat_m2)
    ...
    #dropout_hidden_2 = Dropout(0.5)(hidden_layer_2)
    ...
    output_layer = Dense(1, activation='sigmoid', name='out_sent')(dropout_hidden)

    model = Model([review_text], output=output_layer)
    
    #adam = Adadelta(lr=0.1, rho=0.95, epsilon=1e-6, decay=0.95)
    adam = Adam(0.0001)
    rmsprop = keras.optimizers.RMSprop(lr=0.0005, rho=0.9, epsilon=1e-08, decay=0.0)
...
def gru_model(embedding_weights, cv_dat, max_len, model_w, lda, dictionary, idx2word, alpha, gru_time_steps):
    """
    ...
    #print (train_x.shape)
    #print train_y.shape
    train_x, val_x, train_y, val_y = train_test_split(train_x, train_y, test_size=0.16667, random_state=666, stratify=train_y)
    ...
    callbacks_list = [earlystop]
    print model.summary()
    model.fit([train_x, train_lda], [train_y], batch_size=256, epochs=100,
...
def run_model():
    ...
    #corpus_wordvec = gensim.models.KeyedVectors.load_word2vec_format(word_vec_file, binary=True)
	...
	domn = domain
        revws, labels = get_domain_revs(domn, 'train')
           #print labels
        for r in revws:
		x_tr.append(r)
	for l in labels:
	   ...
	   #	   	x_tr.append(od_r[i])
	   #		y_tr.append(0)
   	rev1k, label1k = get_domain_revs(domn, 'test')
	for r in rev1k:
		x_tr.append(r)
        for l in label1k:
		y_tr.append(l)
        ...
        print np.average([len(sent) for sent in x_tr])
        print np.min([len(sent) for sent in x_tr])
        print "Maximum lenght of sentence in test data:" + str(max_sent_len)
        cv_dat = np.array(x_tr).flatten(), (np.array(y_tr))
        model =  cnn_model(embedding_weights, cv_dat, max_sent_len, idx2word)

        test_dom = [dom for dom in domains if dom != domain]
        for t_d in test_dom:
   	     	x_te, y_te = get_domain_revs(t_d, 'train')
        	x_te1k, y_te1k = get_domain_revs(t_d, 'test')
		...
		transfers = domn + '--->' + t_d
		print transfers
		#dictionary = dictionri
	
  		#test_lda = get_alpha(x_te, lda, dictionary, idx2word, max_len)
		#test_lda1k = get_alpha(x_te1k, lda, dictionary, id2word, max_len)
		max_len=1000

    		test_x = np.array(sequence.pad_sequences(x_te, maxlen=max_len), dtype=np.int)
		test_x1k = np.array(sequence.pad_sequences(x_te1k, maxlen=max_len), dtype=np.int)

 
        #acc, acc1k, misclassified, org_labels = gru_model(embedding_weights, cv_dat, max_sent_len, model_weights_d, lda, dictionri, idx2word, 0.15, 256)
	    	test_predictions = model.predict([test_x], verbose=False)
    		test_predictions1k = model.predict([test_x1k], verbose=False)
    		#test_y = [np.argmax(pred) for pred in test_y]
    		test_pred = [(1 if pred>0.5 else 0) for pred in test_predictions]
    		test_pred1k = [(1 if pred>0.5 else 0) for pred in test_predictions1k]
	    	test_y = [np.argmax(label) for label in y_te]
    		test_y1k = [np.argmax(label) for label in y_te1k]
    		#acc = accuracy_score(test_y, test_pred)
		acc = accuracy_score(y_te, test_pred)
    		print acc
    		acc1k = accuracy_score(y_te1k, test_pred1k)
		print acc1k


        	acc_dict[transfers] = acc 
        	acc_dict1k[transfers] = acc1k
